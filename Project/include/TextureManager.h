...
namespace diva
{
    ...
    public:
        ...
        void draw(std::string id, int x, int y, int width, int height,
                  SDL_Renderer *TRenderer,double angel ,SDL_RendererFlip flip = SDL_FLIP_NONE); // Takes ID and pos where we want to draw the texture, the W&H of the image
        ...
        void drawFrame(std::string id, int x, int y, int widht, int height, int currentRow, int currentFrame,
                       SDL_Renderer *TRenderer, double angel,SDL_RendererFlip = SDL_FLIP_NONE); // dose the same as draw but adds a chceck
